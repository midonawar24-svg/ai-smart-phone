...
class _AIDashboardScreenState extends State<AIDashboardScreen> with SingleTickerProviderStateMixin {
  ...
  for (final model in data["models"]) {
    ...
    if (methods.contains("generateContent")) {
      return (model["name"] as String).replaceFirst("models/", "");
    }
  }
  ...
  Future<void> _processAICommand(String input) async {
    ...
    try {
      ...
      if (_savedApiKey.trim().isNotEmpty) {
        // استدعاء الدالة اللي انت كتبتها
        String? model = await _getAvailableModel();
        
        // لو فشلت، استخدم الأسماء الجديدة كـ fallback
...
''';
          ...
          responseText = '404: الموديل $model مش موجود. جوجل غيرت الأسماء تاني! حاول تاني، الكود هيجيب الاسم الجديد تلقائي.';
        } else {
          String body = res.body;
          if (body.length > 500) body = body.substring(0,500);
          responseText = 'خطأ ${res.statusCode} مع $model: $body';
        }
      } else {
