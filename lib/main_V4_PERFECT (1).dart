...
class _AIDashboardScreenState extends State<AIDashboardScreen> with SingleTickerProviderStateMixin {
  ...
  if (data["models"] == null) return null;
  ...
  Future<void> _processAICommand(String input) async {
    ...
    setState(() {
      _isProcessing = true;
      _thinkingProcess = 'جاري التحليل...\n- المدخل: $input\n- جلب الموديل المتاح (flash/pro فقط)...';
      _aiResponse = 'لحظة...';
    });
    ...
    try {
      ...
      if (_savedApiKey.trim().isNotEmpty) {
        // 1- لا نستخدم fallback ثابت - لو null نرمي Exception
        String? model = await _getAvailableModel();
        ...
        setState(() {
          _thinkingProcess = 'جاري التحليل...\n- الموديل المكتشف: $model\n- ببعت الطلب...';
        });

        Future<http.Response> sendRequest(String m) {
          final url = Uri.parse('https://generativelanguage.googleapis.com/v1beta/models/$m:generateContent?key=$_savedApiKey');
          ...
          return http.post(url, headers: {"Content-Type": "application/json"}, body: body);
        }

        http.Response res = await sendRequest(model);
        
        // 4- لو 404 أو 400 - أعد جلب الموديل وحاول مرة أخرى تلقائياً
        if (res.statusCode == 404 || res.statusCode == 400) {
          setState(() {
            _thinkingProcess = 'جاري التحليل...\n- الموديل $model غير مدعوم (${res.statusCode})\n- بجلب موديل بديل تلقائيا...';
          });
          String? retryModel = await _getAvailableModel();
          if (retryModel != null && retryModel != model) {
            usedModel = retryModel;
            res = await sendRequest(retryModel);
          } else {
            // حاول مرة تانية بنفس الدالة (ممكن ترجع موديل تاني في الترتيب)
            // نلف على الليستة كاملة مرة واحدة كـ fallback ذكي
            try {
              final url = Uri.parse('https://generativelanguage.googleapis.com/v1beta/models?key=$_savedApiKey');
              final listRes = await http.get(url);
              if (listRes.statusCode == 200) {
                ...
                for (final m in data["models"]) {
                  ...
                  if (methods.contains("generateContent") && (name.contains("flash") || name.contains("pro")) && name != model) {
                    usedModel = name;
                    res = await sendRequest(name);
        ...
        if (res.statusCode == 200) {
          final data = jsonDecode(res.body);
          // 3- استخراج الرد بأمان
          responseText = data["candidates"]?[0]?["content"]?["parts"]?[0]?["text"]
              ?? "لم يتم إرجاع أي رد.";
          isReal = true;
        } else if (res.statusCode == 429) {
          responseText = '''
❌ الكوتة خلصت (429) - الموديل: $usedModel

الحل:
1️⃣ استنى دقيقة (15 طلب/دقيقة)
...
''';
          ...
          responseText = 'خطأ ${res.statusCode} مع $usedModel: $body';
        }
      } else {
      ...
      setState(() {
        _thinkingProcess = isReal ? '✓ تم بنجاح ($usedModel)\n- موديل ديناميكي مفلتر flash/pro' : '⚠️ $usedModel';
