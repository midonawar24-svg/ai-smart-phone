...
class _AIDashboardScreenState extends State<AIDashboardScreen> with SingleTickerProviderStateMixin {
  ...
  Future<List<String>> _getAvailableModels() async {
    // الحل الذكي اللي انت اقترحته: نسأل جوجل ايه الموديلات المتاحة
    try {
      final modelsUrl = Uri.parse('https://generativelanguage.googleapis.com/v1beta/models?key=$_savedApiKey');
      final res = await http.get(modelsUrl);
      print("Models list: " + res.body.substring(0, res.body.length > 1000 ? 1000 : res.body.length));
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
        final models = data['models'] as List;
        List<String> candidates = [];
        for (var m in models) {
          final name = (m['name'] as String).replaceAll('models/', '');
          final methods = m['supportedGenerationMethods'] as List? ?? [];
          if (methods.contains('generateContent')) {
            candidates.add(name);
          }
        }
        // ترتيب الأفضلية: الجديد الأول
        ...
        for (var p in preferredOrder) {
          if (candidates.contains(p)) sorted.add(p);
        }
        // ضيف باقي الموديلات اللي فيها flash
        for (var c in candidates) {
          if (!sorted.contains(c) && c.contains('flash')) sorted.add(c);
        }
        // ضيف أي حاجة باقية
        for (var c in candidates) {
    ...
    // fallback لو API بتاع الليست فشل - استخدم الأسماء الجديدة اللي انت قلت عليها
  ...
  Future<void> _processAICommand(String input) async {
    ...
    try {
      ...
      if (_savedApiKey.trim().isNotEmpty) {
        List<String> modelsToTry = await _getAvailableModels();
        setState(() {
          _thinkingProcess = 'جاري التحليل...\n- المدخل: $input\n- الموديلات المتاحة: ${modelsToTry.join(', ')}\n- هجرب أول واحد...';
        });
        
        http.Response? lastRes;
        bool quotaHit = false;
        bool notFoundHit = false;
        ...
        if (!isReal) {
          if (quotaHit) {
            responseText = """
❌ المفتاح خلص الكوتة (429)

جوجل بيقول: You exceeded your current quota

🔧 الحل:
1️⃣ استنى دقيقة وجرب تاني (15 طلب/دقيقة)
2️⃣ اعمل مفتاح جديد من aistudio.google.com/app/apikey بحساب تاني وحطه بدوسة 🔑

الموديلات اللي جربتها: ${modelsToTry.join(', ')}
""";
          } else if (notFoundHit) {
             responseText = "خطأ 404: كل الموديلات اللي جربتها مش موجودة. الموديلات المتاحة حاليا: ${modelsToTry.join(', ')}. جرب تحدث التطبيق.";
             if (lastRes != null) responseText += "\n\nتفاصيل: ${lastRes.body.substring(0, lastRes.body.length > 300 ? 300 : lastRes.body.length)}";
        ...
        responseText = 'تم تحليل: $input\n\n⚠️ ده رد تجريبي. دوس على 🔑 فوق وحط مفتاح Gemini من aistudio.google.com/app/apikey';
      }

      setState(() {
        _thinkingProcess = isReal ? '✓ تم بنجاح ($usedModel)\n- ذكاء حقيقي ديناميكي' : '⚠️ تنبيه\n- الموديل: $usedModel';
      ...
      setState(() {
        ...
        _aiResponse = 'تعذر الاتصال. تأكد من النت والمفتاح. $e';
        _isProcessing = false;
      });
    }
  }
