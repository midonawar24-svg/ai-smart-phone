  ...
  Future<void> _processAICommand(String input) async {
    ...
    try {
      ...
      if (_savedApiKey.trim().isNotEmpty) {
        // Try 2 models for quota fallback
        List<String> models = ['gemini-1.5-flash', 'gemini-2.0-flash', 'gemini-1.5-flash-8b'];
        ...
        if (!isReal) {
          if (quotaHit || (lastRes != null && lastRes.statusCode == 429)) {
            responseText = '''
...
1️⃣ استنى دقيقة وجرب تاني
   المفتاح المجاني له 15 طلب/دقيقة بس

2️⃣ اعمل مفتاح جديد (أسرع حل):
   - روح aistudio.google.com/app/apikey
   - سجل دخول بحساب جوجل تاني
   - دوس Create API Key
   - انسخه وحطه هنا بدوسة 🔑 فوق

3️⃣ قلل الاستخدام:
   متبعتش رسائل ورا بعض بسرعة

💡 نصيحة: اعمل 2-3 مفاتيح وبدل بينهم لما واحد يخلص
''';
          } else if (lastRes != null) {
            // Show trimmed error
            String body = lastRes.body;
            if (body.length > 400) body = body.substring(0,400);
            responseText = 'خطأ ${lastRes.statusCode}: $body';
          }
        }
      } else {
